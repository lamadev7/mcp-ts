...
export const analyzePatientCondition: Tool = {
    ...
    handler: async (params: { patientMessage: string }) => {
        const { patientMessage } = params;
...
export const getCopingStrategies: Tool = {
    ...
    parameters: z.object({
        condition: z
            .string()
            .describe("The mental health condition tag (e.g., 'anxiety', 'depression', 'stress', 'loneliness')"),
    }),
    handler: async (params: { condition: string }) => {
...
export const getTopicDetails: Tool = {
    ...
    handler: async (params: { tag: string }) => {
        ...
        if (!tag?.trim()) {
            return { success: false, error: "Topic tag is required" };
        }
